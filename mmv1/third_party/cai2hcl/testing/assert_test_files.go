...
func AssertTestFiles(t *testing.T, converterNamesPerAssetType map[string]string, converterNamesPerAssetNameRegex map[string]string, converterMap map[string]common.Converter, folder string, fileNames []string) {
	cases := []_TestCase{}
	...
	for i := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			err := assertTestData(c, converterNamesPerAssetType, converterNamesPerAssetNameRegex, converterMap)
			if err != nil {
				t.Fatal(err)
			}
		})
...
func assertTestData(testCase _TestCase, converterNamesPerAssetType map[string]string, converterNamesPerAssetNameRegex map[string]string, converterMap map[string]common.Converter) (err error) {
	...
	got, err := common.Convert(assets, converterNamesPerAssetType, converterNamesPerAssetNameRegex, converterMap)
	if err != nil {
		return err
	}
